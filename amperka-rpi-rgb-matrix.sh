...
# INSTALLER SCRIPT FOR AMPERKA LED RGB DRIVER

# hzeller/rpi-rgb-led-matrix sees lots of active development!
#arduino12/rpi-rgb-led-matrix sees lots of active development!

# That's cool and all, BUT, to avoid tutorial breakage,
# we reference a specific commit (update this as needed):
GITUSER=https://github.com/arduino12
REPO=rpi-rgb-led-matrix
COMMIT=6615a7c7f0d9854fe30fb8802b7f7dcf09f70534
...
echo "This script installs software for the Amperka"
echo "RGB Matrix Driver for Raspberry Pi."
echo "Steps include:"
...
if [ $QUALITY_MOD -eq 1 ]; then
	echo "Reminder: Set jamper to «HQ MODE ON» (QUALITY)"
fi
if [ $QUALITY_MOD -eq 2 ]; then
	echo "Reminder: Remove jamper to «HQ MODE OFF (CONVENIENCE)"
fi
echo
echo -n "CONTINUE? [Y/N] "
read
